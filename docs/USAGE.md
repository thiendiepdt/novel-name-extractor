...
Bạn chỉ cần trình duyệt hiện đại và API key của provider muốn dùng. Không cần cài Node.js nếu chỉ dùng app.

## 2. Lấy API key

Tạo API key trong Google AI Studio, DeepSeek hoặc OpenAI tùy model bạn chọn. App gửi request trực tiếp từ browser tới API của provider tương ứng.
...
Thêm một hoặc nhiều API key đúng với provider của model đang chọn trong phần `Cài đặt`.
...
Chọn model AI ở thanh trên cùng. Với tác vụ trích xuất tên, các model nhanh/lite/nano thường đã đủ. Model đắt hơn không phải lúc nào cũng tốt hơn cho bài toán này.
...
- `Quốc tế`: mặc định giữ tên nhân vật và địa danh nước ngoài theo phiên âm/spelling gốc khi có thể phục hồi từ ngữ cảnh, ví dụ tên Anh/Âu, Nhật, Hàn. Nhóm `Giữ spelling` cho phép chọn thêm hoặc bỏ bớt loại entity được giữ spelling; các loại không được chọn sẽ dùng Hán Việt có dấu để dễ đọc hơn.
...
Số ký tự gửi trong mỗi request AI. Chunk nhỏ giảm nguy cơ timeout nhưng tạo nhiều request hơn. Chunk lớn giảm số request nhưng dễ timeout hoặc vượt response limit hơn.
...
Token và phí trong app là ước lượng. Gemini dùng approximation khoảng 4 ký tự cho 1 token. OpenAI dùng heuristic conservative khoảng 1 token cho mỗi ký tự Hán và 0.25 token cho mỗi ký tự còn lại. DeepSeek dùng heuristic theo tài liệu DeepSeek: khoảng 0.6 token cho mỗi ký tự Hán và 0.3 token cho mỗi ký tự còn lại. Output thực tế có thể lệch theo tokenizer/model, cache hit của DeepSeek và số entity AI trả về.
