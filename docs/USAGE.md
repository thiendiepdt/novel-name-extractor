...
Với test nhỏ, free-tier key có thể đủ. Với truyện dài, nên setup billing và dùng Tier 1. Tier 1 thường nhanh và mượt hơn vì rate limit cao hơn, phù hợp hơn với việc chạy nhiều chunk song song.
...
Nhiều key sẽ được xoay vòng theo request. Điều này giúp khi một key gặp giới hạn tạm thời, nhưng không xóa bỏ limit cơ bản của free tier. Nếu chạy truyện dài hoặc chạy thường xuyên, vẫn nên dùng Tier 1.
...
- `Tây phương`: ưu tiên tên Western/Latin tự nhiên nếu có thể đoán được.

### Gói

- `Miễn phí`: dùng rate limit theo free tier.
- `Tier 1`: dùng rate limit paid tier. Khuyến nghị cho truyện dài.
...
- Free tier: `4000` đến `8000`.
- Tier 1: `6000` đến `12000`, tùy model và raw text.
...
- Free tier: bắt đầu với `1` hoặc `2`.
- Tier 1: có thể tăng cao hơn, nhưng vẫn cần để ý rate limit và quota.
...
Tên có dấu chấm giữa sẽ được normalize khi export:
...
Giảm `Song song`, đợi một lúc, thêm key hợp lệ khác, hoặc chuyển sang Tier 1.

### Timeout

Giảm `Cỡ chunk`, tăng `Timeout (s)`, hoặc dùng Tier 1. App sẽ cố tự tách chunk timeout khi có thể.

### Gemini chặn một chunk

Chunk có thể chứa nội dung kích hoạt safety filter. Thử giảm cỡ chunk, bỏ đoạn nhạy cảm, hoặc chạy với `Không mô tả`.
